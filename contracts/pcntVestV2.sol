...
contract PlayToken is Initializable,OwnableUpgradeable,ERC20PausableUpgradeable{
	...
	function addUserVestingDetails(address _userAddresses, uint8 _vestingIndex, uint256 _totalAmount, uint256 _vestingCliff, uint256 _vestingDuration,uint256 _tgeAmount,uint256 _monthlyAmount) onlyValidVestingBenifciary(_userAddresses,_categoryId) internal{	
	...
	function calculatePrivateSaleTokens(address _userAddresses, uint8 _vestingIndex) internal returns(uint256){
		VestAccountDetails memory vestData = userToVestingDetails[_userAddresses][_vestingIndex];
	 	...
	 	uint8 vestCliff = vestData.vestingCliff;
	 	uint8 vestDuration = vestData.vestingDuration;
		...
		if(currentTime < vestCliff){  // If Vesting Cliff is not reached yet
	 		...
	 		if(totalMonthsElapsed > 2 && totalMonthsElapsed <= 3){
	 			totalClaimableAmount = vestData.monthlyAmount;
	 		}else if(totalMonthsElapsed > 3 && totalMonthsElapsed <= 4){
	 			uint256 totalClaimableAmount = getTokenAmount(vestData.totalAmount,50 ether,100 ether);
	 		}
	 		totalClaimableAmount = totalClaimableAmount.sub(vestData.totalClaimableAmount);
	 	}
	 ...
	 */
	 function calculateClaimableTokens(address _userAddresses,uint8 _vestingIndex) public view checkVestingStatus(_userAddresses,_vestingIndex) returns(uint256){	 	
		...
		if(currentTime < vestCliff){  // If Vesting Cliff is not reached yet
	 		...
	 		totalClaimableAmount = vestData.monthlyAmount.mul(actualMonthElapsed)
	 	}
	 }
